...
def _tfidf_knn_base(
    ...
    """Fit TF-IDF on a sample per country, then run chunked kNN."""
    import gc
    ...
    for country in tqdm(countries, desc=f"kNN({prefix})"):
        ...
        cand_country = cand_df[cand_df["country"] == country]

        if len(s1_country) == 0 or len(cand_country) == 0:
            continue

        # Localize Vectorization
        all_texts_df = pd.concat([s1_country[text_col], cand_country[text_col]]).fillna("")
        ...
        s1_vecs = vec.transform(s1_country[text_col].fillna("")).tocsr()
        cand_vecs = vec.transform(cand_country[text_col].fillna("")).tocsr()

        s1_id_arr = s1_country["entity_id"].values
        cand_id_arr = cand_country["entity_id"].values
        ...
        # Aggressive garbage collection to prevent swap thrashing
        del vec, s1_vecs, cand_vecs, all_texts_df, fit_texts
        gc.collect()
